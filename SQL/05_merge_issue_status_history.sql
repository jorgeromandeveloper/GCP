...
USING (
  ...
  SELECT issue_id, state, ingestion_date
  FROM
    (
      SELECT
        issue_id,
        ...
        ingestion_date,
        ROW_NUMBER()
          OVER (PARTITION BY issue_id ORDER BY ingestion_date DESC) AS rn
...
ON
  ...
  AND T.is_current
    = TRUE
      ...
      WHEN MATCHED AND T.state != S.state THEN UPDATE
SET
  T.valid_to = TIMESTAMP(S.ingestion_date),
  T.is_current = FALSE
    ...
    WHEN NOT MATCHED
      THEN
        INSERT(issue_id, state, valid_from, valid_to, is_current)
          VALUES(S.issue_id, S.state, TIMESTAMP(S.ingestion_date), NULL, TRUE);
